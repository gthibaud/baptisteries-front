...
const Card = () => {
    ...
    const { detachBaptistere, points } = useContext(BaptistereContext);

    return (
        <Map center={[38, 20]} zoom={5} zoomControl={false} onClick={(e) => detachBaptistere()}>
            <TileLayer
            ...
            {points.map(baptistere => <Point data={baptistere} key={baptistere.id} />)}
        </Map>
    );
};
