...
export default function FilterContextProvider({ children }) {
    ...
    const [initialRangeDepth, setInitialRangeDepth] = useState([null, null]);
    const [initialRangePreservedDepth, setInitialRangePreservedDepth] = useState([null, null]);
    ...
    useEffect(() => {
        ...
        setInitialRangeDepth(minMaxDepth);
        const minMaxPreservedDepth = searchMinMaxFields(baptisteriesList, [
            "maximumPreservedDepth",
            "maximumPreservedDepth",
        ]);
        setInitialRangePreservedDepth(minMaxPreservedDepth);
    ...
    const searchMinMaxFields = (baptisteriesList, fieldsToCheck) => {
        const minMax = baptisteriesList
            ...
            .reduce((actualMinMax, newValuesToCheck) => {
                ...
                if (fieldsToCheck[0] === "maximumDepth" || fieldsToCheck[0] === "maximumPreservedDepth") {
                    if (actualMinMax[0] && newValuesToCheck.firstField) {
                        actualMinMax[0] =
                            newValuesToCheck.firstField < actualMinMax[0]
                                ? newValuesToCheck.firstField
                                : actualMinMax[0]
                    } else {
                        if (newValuesToCheck.firstField) {
                            actualMinMax[0] = newValuesToCheck.firstField
                        }
                    }
                } else {
                    actualMinMax[0] =
                        (actualMinMax[0] === undefined || actualMinMax[0] === null) ||
                            newValuesToCheck.firstField < actualMinMax[0]
                            ? newValuesToCheck.firstField
                            : actualMinMax[0]
                }
    ...
    const handleRangeNumber = (event, label, newValues) => {
        event.preventDefault();
        // if (label === 'maximumDepth') {
        //     setInitialRangeDepth(false);
        // }
        // if (label === 'maximumPreservedDepth') {
        //     setInitialRangePreservedDepth(false);
        // }
    ...
    const cancelAllFilters = () => {
        ...
        // setInitialRangeDepth(true);
        // setInitialRangePreservedDepth(true);
        ...
        const newBaptisteriesFiltered = baptisteriesList.filter((baptistery) => {
            return Object.keys(state.filters).every((filter) => {
                ...
                if (filter === "maximumDepth") {
                    ...
                    if (state.filters.maximumDepth[0] === initialRangeDepth[0] &&
                        state.filters.maximumDepth[1] === initialRangeDepth[1]) {
                        return true
                    } else {
                        return (
                ...
                if (filter === "maximumPreservedDepth") {
                    ...
                    if (state.filters.maximumPreservedDepth[0] === initialRangePreservedDepth[0] &&
                        state.filters.maximumPreservedDepth[1] === initialRangePreservedDepth[1]) {
                        return true
                    } else {
                        return (
